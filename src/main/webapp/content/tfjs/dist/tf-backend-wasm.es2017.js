...
(function (global, factory) {
  ...
  function avgPool3D(args) {
    ...
    const convInfo = tfjsCore.backend_util.computePool3DInfo(
      x.shape,
      filterSize,
      strides,
      /*dilations=*/ 1,
  ...
  function avgPoolGrad(args) {
    ...
    const convInfo = tfjsCore.backend_util.computePool2DInfo(input.shape, filterSize, strides, /*dilations=*/ 1, pad);
    const dx = backend.makeOutput(input.shape, input.dtype);
    wasmAvgPoolGrad(
      backend.dataIdMap.get(dy.dataId).id,
      backend.dataIdMap.get(dx.dataId).id,
  ...
  function reshape(args) {
    ...
    tfjsCore.util.assert(
      xSize === tfjsCore.util.sizeFromShape($shape),
  ...
  function concatImpl(inputs, outShape, dtype, simplyConcat) {
    ...
    if (simplyConcat && dtype !== 'string') {
      ...
      let colOffset = 0;
      inputs.forEach(input => {
        const decodedData = dtype === 'string' ? tfjsCore.backend_util.fromUint8ToStringArray(input.vals) : input.vals;
        let tIdx = 0;
  ...
  function sliceImpl(vals, begin, size, shape, dtype) {
    ...
    const decodedData = dtype === 'string' ? tfjsCore.backend_util.fromUint8ToStringArray(vals) : vals;
  ...
  function concat(args) {
    ...
    let outShape = tfjsCore.backend_util.computeOutShape(
    ...
    if ($inputs[0].dtype === 'string') {
      ...
      const finalOutShape = tfjsCore.backend_util.computeOutShape(
        $inputs.map(t => t.shape),
        axis,
      );
  ...
  function conv2d(args) {
    ...
    const convInfo = tfjsCore.backend_util.computeConv2DInfo(
      x.shape,
      filter.shape,
      strides,
      dilations,
  ...
  function conv3DBackpropFilterV2(args) {
    ...
    const convInfo = tfjsCore.backend_util.computeConv3DInfo(x.shape, filterShape, strides, /*dilations=*/ 1, pad);
  ...
  function dilation2D(args) {
    ...
    const dilationInfo = tfjsCore.backend_util.computeDilation2DInfo(
      x.shape,
      filter.shape,
      strides,
      pad,
  ...
  function maxPoolGrad(args) {
    ...
    const { dy, input } = inputs;
    const { filterSize, strides, pad, dimRoundingMode } = attrs;
    const convInfo = tfjsCore.backend_util.computePool2DInfo(input.shape, filterSize, strides, /*dilations=*/ 1, pad, dimRoundingMode);
    const dx = backend.makeOutput(input.shape, input.dtype);
    wasmMaxPoolGrad(
      backend.dataIdMap.get(input.dataId).id,
      backend.dataIdMap.get(dy.dataId).id,
  ...
  function maxPoolWithArgmax(args) {
    ...
    const dilations = [1, 1];
    tfjsCore.util.assert(
      tfjsCore.backend_util.eitherStridesOrDilationsAreOne(strides, dilations),
  ...
  function select(args) {
    ...
    const offset = cRank === 0 || cRank > 1 || tRank === 1 ? 1 : tfjsCore.util.sizeFromShape(t.shape.slice(1));
  ...
  function sparseToDense(args) {
    ...
    const { sliceRank, numUpdates, sliceSize, strides, outputSize } = tfjsCore.backend_util.calculateShapes(
      sparseValues,
      sparseIndices,
      outputShape,
    );
  ...
  function stridedSlice(args) {
    ...
    const {
      ...
      strides: $strides,
    } = tfjsCore.slice_util.sliceInfo(x.shape, begin, end, strides, beginMask, endMask, ellipsisMask, newAxisMask, shrinkAxisMask);
    let result;
    if (isIdentity) {
