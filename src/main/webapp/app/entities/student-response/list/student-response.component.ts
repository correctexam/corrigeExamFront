...
export class StudentResponseComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.studentResponseService
      ...
      .subscribe({
        next: (res: HttpResponse<IStudentResponse[]>) => {
  ...
  trackId(index: number, item: IStudentResponse): number {
    return item.id!;
  }

  delete(studentResponse: IStudentResponse): void {
    const modalRef = this.modalService.open(StudentResponseDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.studentResponse = studentResponse;
  ...
  protected onSuccess(data: IStudentResponse[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
    this.totalItems = Number(headers.get('X-Total-Count'));
    this.page = page;
    if (navigate) {
      this.zone.run(() => {
        this.router.navigate(['/student-response'], {
          queryParams: {
            page: this.page,
            size: this.itemsPerPage,
