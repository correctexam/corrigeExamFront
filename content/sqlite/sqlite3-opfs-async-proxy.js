...
const installAsyncProxy = function (self) {
  ...
  */
  const vfsAsyncImpls = {
    ...
    xLock: async function (fid /*sqlite3_file pointer*/, lockType /*SQLITE_LOCK_...*/) {
      mTimeStart('xLock');
      const fh = __openFiles[fid];
      let rc = 0;
    ...
    xOpen: async function (fid /*sqlite3_file pointer*/, filename, flags /*SQLITE_OPEN_...*/, opfsFlags /*OPFS_...*/) {
      const opName = 'xOpen';
      mTimeStart(opName);
    ...
    xUnlock: async function (fid /*sqlite3_file pointer*/, lockType /*SQLITE_LOCK_...*/) {
      mTimeStart('xUnlock');
      let rc = 0;
      const fh = __openFiles[fid];
    ...
    xWrite: async function (fid /*sqlite3_file pointer*/, n, offset64) {
      ...
      try {
        ...
        rc =
          n === (await getSyncHandle(fh, 'xWrite')).write(fh.sabView.subarray(0, n), { at: Number(offset64) })
            ? 0
  ...
  const waitLoop = async function f() {
    ...
    while (!flagAsyncShutdown) {
      try {
        if ('timed-out' === Atomics.wait(state.sabOPView, state.opIds.whichOp, 0, state.asyncIdleWaitTime)) {
