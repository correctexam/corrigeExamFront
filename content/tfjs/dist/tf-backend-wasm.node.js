...
function avgPool3D(args) {
  ...
  var convInfo = tfjsCore.backend_util.computePool3DInfo(x.shape, filterSize, strides, /*dilations=*/ 1, pad, dimRoundingMode, dataFormat);
...
function reshape(args) {
  ...
  tfjsCore.util.assert(xSize === tfjsCore.util.sizeFromShape($shape), function () {
    return (
      'new shape: '.concat($shape, ', old shape: ').concat(x.shape, '. New shape and old ') + 'shape must have the same number of elements.'
    );
  });
...
function depthwiseConv2d(args) {
  ...
  if (convInfo.dataFormat !== 'channelsLast') {
    throw new Error(
      "wasm backend DepthwiseConv2dNative does not support dataFormat:'" + ''.concat(convInfo.dataFormat, "'. Please use 'channelsLast'."),
    );
  }
  var out = backend.makeOutput(convInfo.outShape, 'float32');
  var outId = backend.dataIdMap.get(out.dataId).id;
